...
public class ClienteChat extends JFrame implements ActionListener, Runnable {
	...
	public ClienteChat(Socket s, String nombre) {
		...
		add(botonSalir);
		botonLimpiar.setBounds(420, 90, 100, 30);
		add(botonLimpiar);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == botonEnviar) { // SE PULSA EL ENVIAR
			...
			if (mensaje.getText().trim().length() == 0) {
				return;
			} else if(mensaje.getText().trim().equals("*")) {
			...
			String texto = nombre + "> " + mensaje.getText().toUpperCase();
		...
		if (e.getSource() == botonLimpiar) { // SE PULSA BOTON LIMPIAR
			try {
				fsalida.writeUTF("");
			} catch (Exception e1) {
				e1.printStackTrace();
